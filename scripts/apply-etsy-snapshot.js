...
if (!Array.isArray(snapshot.listings)) {
  console.error("Snapshot is missing a `listings` array -- see etsy-snapshot.example.json for the expected shape.");
  process.exit(1);
}
...
function isValidRating(r) {
  return !!r &&
    typeof r.value === "number" && r.value >= 0 && r.value <= 5 &&
...
snapshot.listings.forEach(function (item) {
  ...
  if (item.rating && !isValidRating(item.rating)) {
    flags.push(product.name + ": snapshot rating " + JSON.stringify(item.rating) + " failed validation (value must be 0-5, count a non-negative integer) -- ignored, not applied");
  } else {
    var newRating = item.rating && item.rating.count > 0 ? { value: item.rating.value, count: item.rating.count } : null;
    var oldRating = product.rating || null;
    var changed = (!!newRating !== !!oldRating) ||
      (newRating && oldRating && (newRating.value !== oldRating.value || newRating.count !== oldRating.count));
    ...
    if (newRating && changed) {
      ...
      flags.push(product.name + ": snapshot shows 0 reviews but site currently shows " +
        oldRating.value + " (" + oldRating.count + ") -- NOT auto-removed, please verify manually");
    }
  }

  // ---- price drift (reported only, never auto-applied) ----
  if (typeof item.price === "number" && Math.abs(item.price - product.price) > 0.001) {
    flags.push(product.name + ": site shows $" + product.price.toFixed(2) + ", Etsy shows $" + item.price.toFixed(2));
  }
});
...
if (ratingChanges.length) {
  var HEADER = "/* Auto-mirrors assets/data/products.json as a global,\n" +
...
lines.push("Complete pass (safe to check for removed listings): " + (snapshot.complete ? "yes" : "no"));
lines.push("");
...
ratingChanges.forEach(function (c) {
  lines.push("- " + c.name + ": " + (c.from ? c.from.value + " (" + c.from.count + ")" : "no rating yet") +
    " -> " + c.to.value + " (" + c.to.count + ")");
});
lines.push("");
lines.push("## New Etsy listings not on the site yet -- needs a real review, never auto-added (" + newListings.length + ")");
if (!newListings.length) lines.push("_None this run._");
newListings.forEach(function (n) {
  lines.push("- " + (n.title || n.listingId) + " -- $" + (typeof n.price === "number" ? n.price.toFixed(2) : "?") +
    " -- https://www.etsy.com/listing/" + n.listingId);
});
lines.push("");
...
if (!snapshot.complete) {
  ...
  missing.forEach(function (p) { lines.push("- " + p.name + " (" + p.etsyUrl + ")"); });
}
lines.push("");
...
console.log("Missing from this check: " + missing.length + (snapshot.complete ? "" : " (skipped -- incomplete snapshot)"));
