...
PAGES.forEach(function (page) {
  ...
  if (!fs.existsSync(full)) { fail(page, "file missing"); return; }
...
if (!badLinks.length) ok("all internal href/src targets resolve (" + PAGES.length + " pages checked)");
else badLinks.forEach(function (b) { fail("broken reference", b); });

var imgRefs = new Set();
[].concat(
  PAGES.map(function (p) { return path.join(ROOT, p); }),
  ["assets/js/products-data.js", "assets/js/image-manifest.js", "assets/data/products.json"].map(function (p) { return path.join(ROOT, p); })
).forEach(function (f) {
  if (!fs.existsSync(f)) return;
  var text = fs.readFileSync(f, "utf8");
  (text.match(/assets\/img\/[A-Za-z0-9_\-.]+/g) || []).forEach(function (m) { imgRefs.add(m); });
});
var missingImgs = Array.from(imgRefs).filter(function (r) { return !fs.existsSync(path.join(ROOT, r)); });
if (!missingImgs.length) ok("all " + imgRefs.size + " referenced image paths exist on disk");
else missingImgs.forEach(function (m) { fail("missing image file", m); });
...
if (!manifestKeys.length) fail("image-manifest.js", "no entries found");
  ...
  else incomplete.forEach(function (k) { fail("incomplete variants", k); });
  ...
  else missingVariantFiles.forEach(function (f) { fail("manifest references missing file", f); });
}
...
PRODUCTS.forEach(function (p) {
  var missing = REQUIRED_FIELDS.filter(function (f) { return p[f] === undefined || p[f] === null || p[f] === ""; });
  if (!missing.length) ok(p.id + ": required fields present");
  else fail(p.id + ": missing required field(s)", missing.join(", "));

  if (typeof p.price !== "number" || !(p.price > 0)) fail(p.id + ": price must be a positive number", String(p.price));
  ...
  if (p.rating != null) {
    var r = p.rating;
    var ratingOk = r && typeof r.value === "number" && r.value >= 0 && r.value <= 5 &&
  ...
  if (p.stock != null) {
    ...
    else fail(p.id + ": stock must be a non-negative integer (or omitted entirely if not tracked)", JSON.stringify(p.stock));
  }

  if (p.ingredients && p.ingredients.length) {
    var ingOk = Array.isArray(p.ingredients) &&
      p.ingredients.every(function (i) { return typeof i === "string" && i.trim().length > 0; });
    if (ingOk) ok(p.id + ": ingredients well-formed (" + p.ingredients.length + " item(s))");
    else fail(p.id + ": ingredients must be a non-empty array of non-empty strings", JSON.stringify(p.ingredients));
    if (p.ingredientsLabel != null && typeof p.ingredientsLabel !== "string") fail(p.id + ": ingredientsLabel must be a string");
    if (p.ingredientsNote != null && typeof p.ingredientsNote !== "string") fail(p.id + ": ingredientsNote must be a string");
  }
  ...
  if (p.variants) {
    ...
    if (!Array.isArray(v.options) || !v.options.length) {
      ...
      var labels = v.options.map(function (o) { return o.label; });
      var dupeLabels = labels.filter(function (l, i) { return labels.indexOf(l) !== i; });
      if (dupeLabels.length) fail(p.id + ": duplicate variant option labels", dupeLabels.join(", "));
      var hasZeroDelta = v.options.some(function (o) { return (o.priceDelta || 0) === 0; });
      if (!hasZeroDelta) fail(p.id + ": no variant option has priceDelta 0 -- base p.price won't match any option");
      v.options.forEach(function (o) {
        if (typeof o.label !== "string" || !o.label) fail(p.id + ": variant option missing label");
        if (o.priceDelta !== undefined && typeof o.priceDelta !== "number") fail(p.id + ": variant option priceDelta must be numeric", o.label);
        // Snipcart's "Label[+X.XX]" syntax breaks if the label itself
        // contains a literal "[" or "|" -- catch that before it ships.
        if (/[\[\]|]/.test(o.label)) fail(p.id + ": variant option label contains [ ] or | (breaks Snipcart's option syntax)", o.label);
      });
      if (labels.length && !dupeLabels.length) ok(p.id + ": variants well-formed (" + v.name + ": " + labels.join(", ") + ")");
    }
  }
});
...
if (!BUNDLES.length) {
  ...
  BUNDLES.forEach(function (b) {
    ...
    if (!Array.isArray(b.productIds) || b.productIds.length < 2) {
      ...
      var missingIds = b.productIds.filter(function (id) { return productIds.indexOf(id) === -1; });
      if (missingIds.length) problems.push("references unknown product id(s): " + missingIds.join(", "));
    }
...
if (fs.existsSync(snipcartPath)) {
  ...
  snipcartManifest.forEach(function (p) {
    (p.customFields || []).forEach(function (cf) {
      ...
      var parsedOk = parts.every(function (part) { return /^.+\[[+-]\d+\.\d{2}\]$/.test(part); });
      if (parsedOk) ok(p.id + ": customField \"" + cf.name + "\" options parse correctly (" + parts.length + " option(s))");
      else fail(p.id + ": customField \"" + cf.name + "\" options don't match Label[+X.XX] pattern", cf.options);
    });
  });
} else {
...
function resolveHex(tokens, value, depth) {
  ...
  if (varMatch) return tokens[varMatch[1]] ? resolveHex(tokens, tokens[varMatch[1]], depth + 1) : null;
...
[["dark", darkTokens], ["light", lightTokens]].forEach(function (pair) {
  var theme = pair[0], tokens = pair[1];
  PAIRS.forEach(function (spec) {
    var fgHex = resolveHex(tokens, tokens[spec[0]] || "");
    var bgHex = resolveHex(tokens, tokens[spec[1]] || "");
    if (!fgHex || !bgHex) { fail("[" + theme + "] " + spec[3], "couldn't resolve --" + spec[0] + " or --" + spec[1] + " to a hex value"); return; }
    var ratio = contrastRatio(fgHex, bgHex);
    var label = "[" + theme + "] " + spec[3] + " (" + fgHex + " on " + bgHex + "): " + ratio.toFixed(2) + ":1";
...
if (!domainHits.length) {
  ...
  fail(
    ...
    domainHits.join(", ") + " -- set a real DOMAIN in scripts/build-site-data.js and re-run npm run build-data"
  );
}
...
function extractVercelCSP() {
  ...
  var entry = (group || []).find(function (h) { return h.key === "Content-Security-Policy"; });
...
try {
  ...
  if (!cspHeaders || !cspVercel || !cspNetlify) {
    ...
    fail(
      ...
      "_headers " + (cspHeaders === cspVercel ? "==" : "!=") + " vercel.json, " +
      "vercel.json " + (cspVercel === cspNetlify ? "==" : "!=") + " netlify.toml -- run npm run build-security-headers to resync"
    );
  }
} catch (e) {
...
var shopLdMatch = shopHtml.match(/<script type="application\/ld\+json">\n(\{\s*\n\s*"@context": "https:\/\/schema\.org",\s*\n\s*"@type": "ItemList"[\s\S]*?\n)<\/script>/);
if (!shopLdMatch) {
  ...
  try {
    ...
    items.forEach(function (li) {
      ...
      var good = ar["@type"] === "AggregateRating" && val >= 0 && val <= 5 && Number.isInteger(cnt) && cnt > 0;
      if (good) ok(prod.name + ": aggregateRating well-formed (" + val + " / " + cnt + ")");
      else fail(prod.name + ": aggregateRating malformed", JSON.stringify(ar));
    });
    if (!ratedCount) console.log("  (no products currently carry aggregateRating -- fine if none have real reviews yet)");
    var productsWithDataRating = PRODUCTS.filter(function (p) { return p.rating; }).length;
    if (ratedCount === productsWithDataRating) {
      ok("aggregateRating present for exactly the " + ratedCount + " product(s) with real rating data -- none fabricated, none missing");
    } else {
      fail("aggregateRating count mismatch", ratedCount + " in JSON-LD vs " + productsWithDataRating + " products with rating data in products-data.js -- run npm run build-data");
    }
  } catch (e) {
...
if (!SITE_REVIEWS.length) {
  ...
  SITE_REVIEWS.forEach(function (r, i) {
    ...
    if (r.productId !== null && r.productId !== undefined && productIds.indexOf(r.productId) === -1) {
      problems.push("productId \"" + r.productId + "\" doesn't match any product in products-data.js");
    }
    if (r.date !== undefined && !/^\d{4}-\d{2}-\d{2}$/.test(r.date)) problems.push("date must be YYYY-MM-DD");
...
PAGES.forEach(function (page) {
  ...
  var offenders = comments.filter(function (c) { return /<script\b/i.test(c); });
  if (!offenders.length) ok(page + ": no comment contains a literal \"<script\"");
  else fail(page + ": HTML comment contains literal \"<script\"", "this breaks build-security-headers.js's naive tag scanner -- reword the comment to avoid the exact substring \"<script\"");
});

/* ---------- 14) CSP covers every currently-wired third-party integration ---------- */
section("CSP covers every currently-wired integration domain");
var cspText = null;
try { cspText = extractHeadersFileCSP(); } catch (e) { /* handled below */ }
if (!cspText) {
  ...
  REQUIRED_CSP_SUBSTRINGS.forEach(function (pair) {
    ...
    else fail("CSP missing " + pair[0], pair[1] + " is wired into the site but not allowlisted -- it'll be silently blocked by the browser");
  });
}
...
PAGES.forEach(function (page) {
  ...
  if (!m) { fail(page + ": Tawk.to placeholder script", "not found"); return; }
  tawkTexts[page] = m[0];
...
if (Object.keys(tawkTexts).length === PAGES.length && tawkUnique.length === 1) {
  ...
  fail("Tawk.to placeholder script text diverges between pages", "found " + tawkUnique.length + " distinct version(s) -- should be exactly 1");
}
...
if (/id="giftUpContainer"/.test(shopHtml)) {
  ...
  if (/YOUR_GIFTUP_EMBED_CODE/.test(shopHtml)) {
    console.log("  (still the placeholder -- expected until Savanna has a real Gift Up! account, see README section 18)");
  } else {
    ok("placeholder text has been replaced with a real embed (or something else) -- if this is a real Gift Up! snippet, nice");
  }
} else {
  fail("shop.html", "#giftUpContainer not found -- gift-card section may have been removed or renamed");
}
...
else fail("main.js", "doesn't reference #shopSearch -- search box would be inert markup with no listener");
...
(function () {
  if (!FAQ.length) { fail("products-data.js CATALOG.faq", "no questions found"); return; }
  var seenQuestions = {};
  FAQ.forEach(function (item, i) {
    var label = "faq[" + i + "]" + (item.question ? ' "' + item.question + '"' : "");
    var problems = [];
    if (!item.question || typeof item.question !== "string") problems.push("missing/invalid question");
  ...
  var ldMatch = contactHtml.match(/<script type="application\/ld\+json">\n(\{\s*\n\s*"@context": "https:\/\/schema\.org",\s*\n\s*"@type": "FAQPage"[\s\S]*?\n)<\/script>/);
  if (!ldMatch) {
    ...
    try {
      ...
      if (JSON.stringify(actualLd) === JSON.stringify(expectedLd)) {
        ok("contact.html FAQPage JSON-LD matches products-data.js's faq array (" + FAQ.length + " question(s))");
      } else {
        fail("contact.html FAQPage JSON-LD is stale", "doesn't match products-data.js's faq array -- run npm run build-data");
      }
    } catch (e) {
  ...
  if (!markerMatch) {
    ...
    var expectedItems = FAQ.map(function (item) { return { q: item.question, a: item.answer }; });
    if (JSON.stringify(visibleItems) === JSON.stringify(expectedItems)) {
      ...
      fail("contact.html visible FAQ prose is stale", "doesn't match products-data.js's faq array -- run npm run build-data");
    }
  }

  // Regression guard: shop.html should link to the FAQ, not duplicate it.
  if (/class="faq-accordion"/.test(shopHtml)) {
    fail("shop.html", "still has a .faq-accordion -- the site is only supposed to have one FAQ now (see contact.html#faq); remove the duplicate");
  } else if (/href="contact\.html#faq"/.test(shopHtml)) {
    ok("shop.html links to contact.html#faq instead of duplicating FAQ content");
  } else {
    fail("shop.html", "doesn't appear to link to contact.html#faq -- the \"check our FAQ before checkout\" pointer may have been removed");
  }
})();
...
if (!BUNDLES.length) {
  ...
  fail("bundle pricing freshness", "assets/data/snipcart-products.json missing -- run npm run build-data");
} else {
  var PRODUCTS_BY_ID_QA = {};
  PRODUCTS.forEach(function (p) { PRODUCTS_BY_ID_QA[p.id] = p; });
  var snipcartById = {};
  JSON.parse(fs.readFileSync(snipcartPath, "utf8")).forEach(function (item) { snipcartById[item.id] = item; });
  BUNDLES.forEach(function (b) {
    var missing = b.productIds.filter(function (id) { return !PRODUCTS_BY_ID_QA[id]; });
    if (missing.length) return; // already reported as a failure in section 6b
    var fullPrice = b.productIds.reduce(function (sum, id) { return sum + PRODUCTS_BY_ID_QA[id].price; }, 0);
    ...
    if (!snipcartEntry) {
      ...
      ok("bundle-" + b.id + ": snipcart-products.json price ($" + snipcartEntry.price.toFixed(2) + ") matches fresh recompute from products-data.js");
    } else {
      fail("bundle-" + b.id + " price drift", "snipcart-products.json has $" + snipcartEntry.price + " but products-data.js currently computes $" + expectedPrice.toFixed(2) + " -- run npm run build-data");
    }
  });
}
...
if (!fs.existsSync(productsJsonPath)) {
  ...
  try {
    ...
    if (JSON.stringify(generatedCatalog) === JSON.stringify(canonicalCatalog)) {
      ok("assets/js/products-data.js's window.YL_PRODUCTS deep-equals a fresh parse of products.json");
    } else {
      fail(
...
if (!fs.existsSync(configYmlPath)) {
  ...
  else fail("admin/config.yml", "backend.name isn't \"github\" -- expected the GitHub backend (see README section 20)");

  if (/repo:\s*YOUR_GITHUB_USERNAME\/YOUR_REPO_NAME/.test(configYml)) {
    console.log("  (repo is still the YOUR_GITHUB_USERNAME/YOUR_REPO_NAME placeholder -- expected until a real GitHub repo exists, see README section 20)");
  ...
  if (/file:\s*assets\/data\/products\.json/.test(configYml)) ok("file collection points at assets/data/products.json");
  else fail("admin/config.yml", "doesn't reference assets/data/products.json -- the CMS wouldn't be editing the real catalog file");

  if (/media_folder:\s*\/assets\/img/.test(configYml) && /public_folder:\s*\/assets\/img/.test(configYml)) {
  ...
  if (fs.existsSync(productsJsonPath)) {
    try {
      ...
      topLevelKeys.forEach(function (key) {
        ...
        if (fieldRe.test(configYml)) ok("config.yml defines a field for products.json's \"" + key + "\" key");
        else fail("admin/config.yml", "no \"- name: " + key + "\" field found -- products.json has this top-level key but the CMS has no field for it");
      });
    } catch (e) { /* already reported as a failure above */ }
  }
}
...
function extractVercelAdminCSP() {
  var json = JSON.parse(fs.readFileSync(path.join(ROOT, "vercel.json"), "utf8"));
  var rule = (json.headers || []).find(function (h) { return h.source === "/admin/(.*)"; });
  var entry = rule && (rule.headers || []).find(function (h) { return h.key === "Content-Security-Policy"; });
  return entry ? entry.value : null;
}
...
try {
  ...
  if (!adminCspHeaders || !adminCspVercel || !adminCspNetlify) {
    fail("could not extract /admin/* CSP from all three files", "run npm run build-security-headers first");
    ...
    fail(
      ...
      "_headers " + (adminCspHeaders === adminCspVercel ? "==" : "!=") + " vercel.json, " +
      "vercel.json " + (adminCspVercel === adminCspNetlify ? "==" : "!=") + " netlify.toml -- run npm run build-security-headers to resync"
    );
  }
  if (adminCspHeaders && cspHeaders && adminCspHeaders === cspHeaders) {
    fail("/admin/* CSP is identical to the main-site CSP", "expected a separate, more permissive-for-Sveltia policy -- see build-security-headers.js's adminCsp");
