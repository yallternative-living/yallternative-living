...
/* Netlify Functions that no longer exist. Their code moved into the Worker
   (audit C-1 deleted redeem-points outright), and this repository is the
   publish root, so without these rules the paths would 404 as ordinary missing
   files. 410 Gone is the difference that matters: a 404 says "try again later",
   a 410 tells a cached client, a crawler and a stale service worker to stop
   asking. See docs/STATE-LAYER.md for the route each one became. */
var RETIRED_FUNCTIONS = [
...
function run() {
  ...
  var netlifyToml =
    ...
    // Emitted BEFORE the /api/* proxy and before the blocked-path rules,
    // because Netlify applies the first matching rule and these are the most
    // specific paths in the file.
    "# The four Netlify Functions this site used to run are gone -- the money\n" +
    "# path is one Cloudflare Worker now (workers/checkout.js). 410, not 404:\n" +
    "# a cached client or a crawler holding one of these URLs should stop\n" +
    "# asking, not retry. Generated by scripts/build-security-headers.js.\n" +
    RETIRED_FUNCTIONS.map(function (retiredPath) {
      return (
        "[[redirects]]\n" +
        '  from = "' +
        retiredPath +
    ...
    // function URLs under /.netlify/functions/ are answered 410 above.
