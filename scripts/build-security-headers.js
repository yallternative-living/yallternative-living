...
/* "/journal/*" is the per-post pages (journal/<slug>.html). build-site-data.js
   removes them while the flag is off, but a CDN can still be holding one
   from the previous deploy, so the rule closes the whole prefix. */
var JOURNAL_PATHS = ["/journal.html", "/feed.xml", "/journal/*"];
...
function shippedHtmlPages() {
  ...
  // The generated pages: products/<id>.html and journal/<slug>.html.
  ["products", "journal"].forEach(function (sub) {
...
function allPages() {
  ...
  // products/*.html and journal/*.html are both generated by build-site-data.js
  // and both carry the theme-init inline script; glob them so none is unhashed.
  ["products", "journal"].forEach(function (sub) {
    var dir = path.join(ROOT, sub);
    if (!fs.existsSync(dir)) return;
    fs.readdirSync(dir)
...
function run() {
  ...
  var netlifyToml =
    ...
    // that reach main change nothing Netlify serves -- CI workflow tweaks,
    // docs, the Cloudflare Worker (deployed by Cloudflare, not here), test
    // files -- so the build now refuses to spend credits on
    // them. git's exit code IS the decision: 0 (no difference outside the
    // excluded paths) cancels the build, anything else builds.
    //
    // The two tests in front of git are the safe default, and the first
    // version of this rule did not have them. Measured 2026-09-05: with
    // CACHED_COMMIT_REF empty the shell drops the word and `git diff --quiet
    // <commit> -- ...` compares the commit to the (clean) checkout -- exit 0,
    // build SKIPPED; with CACHED_COMMIT_REF equal to COMMIT_REF (Netlify's
    // first build, or a cleared cache) `git diff --quiet X X` is exit 0 too --
    // build SKIPPED. The comment here used to promise the opposite. Either
    // test failing exits 1, and 1 builds. `x` prefixes keep `[` well-formed
    // when a value is empty, without quotes that TOML would need escaping. Excludes only what is certainly not part of
    ...
    "\n" +
    "  # Skip the build -- and its credits -- when nothing that ships changed.\n" +
    ...
    "  # builds, and the two tests up front make sure of that when there is no\n" +
    "  # cached commit to diff against (first build, cleared cache): without\n" +
    "  # them git compared the checkout to itself and skipped. Excludes only what is\n" +
    "  # certainly not served; when in doubt leave a path out of this list.\n" +
    "  # Generated by scripts/build-security-headers.js.\n" +
    "  ignore = \"[ x$CACHED_COMMIT_REF != x ] && [ x$CACHED_COMMIT_REF != x$COMMIT_REF ] && git diff --quiet $CACHED_COMMIT_REF $COMMIT_REF -- . ':(exclude)docs' ':(exclude)workers' ':(exclude).github' ':(exclude)*.md' ':(exclude)scripts/*.test.js'\"\n\n" +
    ...
    "#\n" +
    // Netlify meters this site in credits -- every build spends them -- and
    // this account's allowance ran out on 2026-09-04, so anything a build
    // downloads and never opens is worth deleting. Asking for
    // devDependencies (above) also installs puppeteer
    ...
    "# images, rebuilds site data and rewrites these headers -- so those\n" +
    "# bytes are credits bought and thrown away. Netlify meters this site in\n" +
    "# credits, and every build spends them; this account's allowance ran out\n" +
    "# on 2026-09-04. These two lines are credits back. CI and laptops set\n" +
    "# neither, so the tests still get real browsers.\n" +
    "#\n" +
