...
_(Note: I bumped your catalog prices up slightly from Etsy to match current market rates. You can always change them in the dashboard.)_
...
- **Where to go:** `https://yallternativeliving.com/admin` _(this link will be active once your site is officially live)_
- **What you can manage here:**
  ...
  - **Markets & Pop-Ups:** Add upcoming market dates (which automatically promote to "Where We've Been" after the event date passes) or archive past dates.
...
- **Two services deploy this site, and they bill separately**: **Netlify**
  ...
  question is always _which one_. Netlify is already set to build only `main`
...
- **Automated Verification**:
  - `npm test` runs the Node-only unit pool -- 47 non-browser `scripts/*.test.js` suites
    (49 with verification gates) covering cart and checkout pricing, tax, gift cards,
    ...
    `scripts/qa-check.js`, and its 1129 assertions: links, images, JSON-LD,
