...
(async () => {
  ...
  try {
    ...
    const hrefs = await page.$$eval('a', links => links.map(a => a.href).filter(h => h.startsWith('http')));
    let brokenLinks = [];
    ...
    if (menuToggle) {
      ...
      const isActive = await page.$eval('.nav-links', el => el.classList.contains('active') || window.getComputedStyle(el).display !== 'none');
    ...
    if (addBtn) {
      ...
      try {
        ...
        snipcartVisible = await page.$eval('#snipcart', el => {
