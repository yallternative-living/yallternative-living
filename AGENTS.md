...
7. **Every commit on `main` is a paid production deploy**:
   - Netlify charges **15 credits per production deploy** against a **300-credit month**: the plan pays for 20 deploys of `main`. By 4 September 2026 the billing period had already counted 66 production deploys -- one per commit that reached `main`, 990 credits -- and once they were spent the site could not deploy at all (it kept serving the last published build, which is the one mercy of the model). Six of those landed on one day from two agent sessions merging separately, three of them changing only `scripts/` or `.github/`.
   - **Skip what does not ship.** `netlify.toml` carries a `[build] ignore` command: `git diff --quiet $CACHED_COMMIT_REF $COMMIT_REF -- . ':(exclude)docs' ':(exclude)workers' ':(exclude).github' ':(exclude)*.md' ':(exclude)scripts/*.test.js'`. git exits 0 when the diff since the last deployed commit touches only those paths, and Netlify cancels a build whose ignore command exits 0 -- no deploy, no 15 credits. Anything else builds, including a missing `CACHED_COMMIT_REF` (first build, cleared cache), which is the safe default. The rule is generated by `scripts/build-security-headers.js`; change it there, never in the toml by hand. Excluding a path that IS served would make a real change silently never deploy, so the list holds only what is certainly not part of the deploy (the Worker is deployed by Cloudflare, docs and workflows are never served, `*.test.js` never runs on Netlify); when unsure, leave a path out of it and pay for the build.
   - **Batch.** One merge per body of work, not one per commit; when two sessions are open, land both on one branch and merge once. A push to `main` that could have waited for the next one cost 5% of the month.
   - A push to a PR branch is a deploy-preview build. Put `[skip netlify]` in the commit message when the preview is not needed (CI's `browser` job is the gate, not the preview), and note that it does not skip GitHub Actions -- `[skip ci]` would, so never use that one.
