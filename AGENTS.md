...
> **What CI actually runs**: `.github/workflows/test.yml` runs steps 5, 6 and 4 only -- `npm run lint`, `npm run format:check`, `npm test`. It deliberately sets `PUPPETEER_SKIP_DOWNLOAD` to skip Chromium, so **`npm run test:integration` does not run on push**: the accessibility gate, the scroll-reveal gate and the Puppeteer suites are local-only, and a regression in any of them reaches `main` with a green board. Run step 7 yourself before finalising any change that touches CSS, `assets/js/*.js` or page markup. (Static assertions in `scripts/qa-check.js` *are* covered by CI, which is why the feature-flag single-source-of-truth check lives there rather than in a browser suite.)
